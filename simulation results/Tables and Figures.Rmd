...
cc_same_rel_bias <- cc_same %>% 
  dplyr::select(model, mobility, J, rho, tau, rel_bias) %>% 
  pivot_wider(., names_from = "model", values_from = "rel_bias") 
  
mm_same_rel_bias %>% 
  left_join(cc_same_rel_bias, 
...
```
  ...
  > The relative parameter bias of MMREM is still too large, but the values of CCREM seem reasonable, especially when rho = FALSE. It makes sense because CCREM should be good when there is no correlation. 
