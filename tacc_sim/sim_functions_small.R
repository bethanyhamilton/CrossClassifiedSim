...
run_mmrem <-
  function(data,
    ...
    summary$stat <- row.names(summary)
    summary$param <- "sigma_u2"
    summary$model <- "mmrem"
    summary$mobility <- mobility
    summary$J <- J
    summary$rho <- rho
    summary$tau <- tau
...
runSim <- function(reps, mobility, J, rho, tau, mmrem=TRUE, ccrem=FALSE, rccrem=FALSE, seed = NULL, ...){
